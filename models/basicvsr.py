...
from core.modules import ResidualBlock, PixelShufflePack

class BasicVSR(nn.Module):
    def __init__(self, mid_channels=64, blocks=30, upscale=4, is_mirror=False):
        super().__init__()
        self.name = 'BasicVSR'
        self.is_mirror = is_mirror
        self.mid_channels = mid_channels
        self.spynet = Spynet().requires_grad_(False)
        self.backward_resblocks = ResidualBlock(mid_channels + 3, mid_channels, blocks)
        self.forward_resblocks = ResidualBlock(mid_channels + 3, mid_channels, blocks)
